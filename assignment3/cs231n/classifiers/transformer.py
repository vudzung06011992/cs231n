...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        position_emb = self.positional_encoding(captions_emb)
        
        captions_emb = captions_emb + position_emb
        
        img_emb = self.visual_projection(features)
        img_emb = torch.unsqueeze(img_emb, dim=-2)

        tgt_mask = torch.tril(torch.ones((T, T)), diagonal=-1)
        tgt_mask = tgt_mask == 0
        
        scores = self.transformer(captions_emb, img_emb, tgt_mask=tgt_mask)
