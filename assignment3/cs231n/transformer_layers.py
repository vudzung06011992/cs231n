...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        key = key.view(N, T, self.n_head, self.head_dim).permute(0, 2, 3, 1)
        value = value.view(N, T, self.n_head, self.head_dim).permute(0, 2, 1, 3) #N, n_head, T, Width

        
        e = torch.matmul(query, key)/(self.head_dim)**0.5
        if attn_mask is not None:
          e = e.masked_fill(attn_mask==0, -1e9)
        a = torch.softmax(e, dim=-1) #(N, n_head, S, T)
        a = self.attn_drop(a)

        # tinh xong attention
        
        head = torch.matmul(a, value) #(N, n_head, S, width)
        head = head.permute(0, 2, 1, 3)
        output = head.reshape(N, S, E)
