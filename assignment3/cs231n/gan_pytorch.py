...
def sample_noise(batch_size, dim, seed=None):
    ...
    x = torch.FloatTensor(batch_size, dim).uniform_(-1, 1)
...
def generator(noise_dim=NOISE_DIM, seed=None):
    ...
    model = nn.Sequential(
        torch.nn.Linear(in_features=noise_dim, out_features=1024,bias=True),
        torch.nn.ReLU(),
        torch.nn.Linear(in_features=1024, out_features=1024,bias=True),
        torch.nn.ReLU(),
        torch.nn.Linear(in_features=1024, out_features=784,bias=True),
        torch.nn.Tanh()
    )
...
def discriminator_loss(logits_real, logits_fake):
    ...
    P_real = 1 / ( 1 + torch.exp(-logits_real))
    P_fake = 1 / ( 1 + torch.exp(-logits_fake))

    loss = - torch.mean(torch.log(P_real)) - torch.mean(torch.log(1 - P_fake))
...
def generator_loss(logits_fake):
    ...
    P_fake = 1 / ( 1 + torch.exp(-logits_fake))
    loss = - torch.mean(torch.log(P_fake))
...
def build_dc_classifier(batch_size):
    ...
    model = nn.Sequential(
        ...
        nn.MaxPool2d(kernel_size=(2,2), stride=2),
        nn.Flatten(),
        nn.Linear(1024, out_features=4*4*64),
        nn.LeakyReLU(0.01),
        nn.Linear(4*4*64, out_features=1)
