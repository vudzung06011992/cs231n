...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        if i == T-1:
            ...
            # upstream gradient se den tu hai nguon: (1) la tu hidden state lien truoc, (2) la tu loss tương ứng với output tại ngay timestep do.
            dnext_h = dh[:, i, :] + dprev_h_i
        dx_i, dprev_h_i, dWx_i, dWh_i, db_i = rnn_step_backward(dnext_h, cache_list["{}".format(i)])
        dx[:, i, :] = dx_i
        dWx += dWx_i
        dWh += dWh_i
        db += db_i
...
def word_embedding_backward(dout, cache):
    ...
    x_t = x.flatten()
    dout_t = dout.reshape((N*T, D))
    dW = np.zeros(W.shape)
    np.add.at(dW, x_t, dout_t)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, prev_h, prev_c, i, f, o, g, next_c, next_h, Wx, Wh, b)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    Inputs:
    - dnext_h: Gradients of nex/t hidden state, of shape (N, H)
    ...
    # => next_h = o * np.tanh(next_c)
    x, prev_h, prev_c, i, f, o, g, next_c, next_h, Wx, Wh, b = cache
    ...
    da = np.concatenate([da_from_i, da_from_f, da_from_o, da_from_g], axis=1)
...
def lstm_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        if i == T-1:
            # hidden state chi co mot gradient contribution tu loss
            dnext_h = dh[:, i, :]
        else:
            # hidden state co gradient contribution tu loss va tu tu hidden state tiep theo
            dnext_h = dh[:, i, :] + dprev_h_i            
        dx_i, dprev_h_i, dprev_c_i, dWx_i, dWh_i, db_i = lstm_step_backward(dnext_h, dnext_c, cache_list["{}".format(i)])
        dx[:, i, :] = dx_i
        dWx += dWx_i
        dWh += dWh_i
        db += db_i
